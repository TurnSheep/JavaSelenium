...
public class HtmlReportFormatter extends Formatter {
	...
	public String format(LogRecord rec) {
	   ...
	   // if(RecordStore.result.equalsIgnoreCase("Pass"))	{	//忽略大小写
	...
	{
		this.setStartTime = System.currentTimeMillis(); //注入开始时间
	...
	{
		this.setEndTime = System.currentTimeMillis(); //注入结束时间
		...
		if (p_total > 0)
			if (RecordStore.p_fail > 0)
				...
				HTML_Tail = "</table></PRE>" + "<br>&nbsp;开始时间   ："+ HtmlUtil.getCalcDate(this.setStartTime) 
						...
						+ "<br>&nbsp;执行用例      ：" + p_total  //total值 p_pass+p_fail
						+"<br>&nbsp;用例成功         ："+ RecordStore.p_pass   //format 中传入值
						+ "<br>&nbsp;<font color=Red>用例失败      ："+ RecordStore.p_fail + "</font>"  //format 中传入值
